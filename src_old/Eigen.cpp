...
Eigen::Eigen(arma::cx_mat& SS, arma::cx_mat& TT, size_t& numt, size_t& numz, double& kk, int& numModes)
{
    ...
    //             TT(arma::span(numt,numtot-1),arma::span(0,numt-1)), "standard");
    arma::cx_mat AA = -TT(arma::span(numt,numtot-1),arma::span(numt,numtot-1)); // -D^-1
    arma::cx_mat BB = TT(arma::span(numt,numtot-1),arma::span(0,numt-1)); // C^T
    arma::cx_mat XX = arma::solve(AA, BB); // C^T q_z <-- -D^-1 C^T q_t
    OO(arma::span(numt,numtot-1),arma::span(0,numt-1)) = XX;
